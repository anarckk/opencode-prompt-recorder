...
const PACKAGE_NAME = "opencode-prompt-recorder"
    ...
    .then((result) => {
      ...
      setTimeout(() => {
        ctx.client.tui.showToast({
          body: {
            title: "Prompt Recorder 更新",
            message: `${result.name} 已从 ${result.current} 更新到 ${result.latest}，重启 OpenCode 完成更新`,
            variant: "info",
          },
        })
      }, 5000)
...
async function findPackageDir(name: string): Promise<string | undefined> {
  ...
  for (;;) {
    ...
    if (pkg?.name === name) {
      if (basename(dir) === "dist") return dirname(dir)
      return dir
    }
...
function isAutoUpdatableSpec(spec: string) {
